...
import { CONSENT_STATUSES, type ConsentStatus } from "@/lib/validation/volunteer";
...
function toAdminVolunteer(input: unknown): AdminVolunteer {
  ...
  for (const key of SOCIAL_KEYS) {
    ...
    if (entry && typeof entry === "object") {
      const { url, approved } = entry as { url?: unknown; approved?: unknown };
      const safe = safeExternalUrl(url);
      if (safe) socialLinks[key] = { url: safe, approved: approved === true };
    }
  }
  return {
    id: row.id,
...
export async function listAdminVolunteers(supabase: SupabaseClient, query: AdminVolunteerQuery) {
  ...
  if (query.team === "none") request = request.is("team_id", null);
  else if (query.team && isUuid(query.team)) request = request.eq("team_id", query.team);
  ...
  if (error) {
    if (error.code === "PGRST103") return { volunteers: [] as AdminVolunteer[], total: count ?? 0, pageCount: 1 };
    throw new AdminDataError("Could not load volunteers.", { cause: error });
  }
...
export async function getAdminVolunteer(supabase: SupabaseClient, id: string): Promise<AdminVolunteer | null> {
  if (!isUuid(id)) return null;
  const { data, error } = await supabase.from("volunteers").select(ADMIN_COLUMNS).eq("id", id).maybeSingle();
  if (error) throw new AdminDataError("Could not load the volunteer.", { cause: error });
...
const teamRowSchema = z.object({
  ...
  is_active: z.boolean(),
  updated_at: z.string(),
  volunteers: z.array(z.object({ count: z.number() })),
});
...
export async function listAdminTeams(supabase: SupabaseClient): Promise<AdminTeam[]> {
  const { data, error } = await supabase
    .from("teams")
    .select("id, name, slug, description, is_active, updated_at, volunteers(count)")
    .order("name", { ascending: true });
  if (error) throw new AdminDataError("Could not load teams.", { cause: error });
  return z.array(teamRowSchema).parse(data).map((t) => ({
    id: t.id,
    name: t.name,
    slug: t.slug,
...
export async function listTeamOptions(supabase: SupabaseClient) {
  const { data, error } = await supabase.from("teams").select("id, name, is_active").order("name", { ascending: true });
  if (error) throw new AdminDataError("Could not load teams.", { cause: error });
  return z
    .array(z.object({ id: z.string(), name: z.string(), is_active: z.boolean() }))
    .parse(data)
    .map((t) => ({ id: t.id, name: t.name, isActive: t.is_active }));
}

export type DashboardCounts = { total: number; published: number; draft: number; consentMissing: number; teams: number };

export async function getDashboardCounts(supabase: SupabaseClient): Promise<DashboardCounts> {
  const head = { count: "exact" as const, head: true };
  const [total, published, consentMissing, teams] = await Promise.all([
    supabase.from("volunteers").select("id", head),
    supabase.from("volunteers").select("id", head).eq("is_published", true),
    supabase.from("volunteers").select("id", head).neq("consent_status", "granted"),
    supabase.from("teams").select("id", head),
  ]);
  const failed = [total, published, consentMissing, teams].find((r) => r.error);
  if (failed?.error) throw new AdminDataError("Could not load dashboard counts.", { cause: failed.error });
  const t = total.count ?? 0;
  const p = published.count ?? 0;
  return { total: t, published: p, draft: t - p, consentMissing: consentMissing.count ?? 0, teams: teams.count ?? 0 };
}
...
export async function signPhotoUrls(supabase: SupabaseClient, paths: string[]): Promise<Record<string, string>> {
  ...
  const { data, error } = await supabase.storage.from(PHOTO_BUCKET).createSignedUrls(unique, 60 * 60);
